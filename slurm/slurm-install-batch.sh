...
ARCH=$(/modules/spack/bin/spack arch)
echo jobid $SLURM_JOB_ID on host $(hostname) with arch $ARCH by user $(whoami) on $(date)
...
echo spack install --verbose -y $SPACK_INSTALL_ARGS arch=$ARCH
spack install --verbose -y $SPACK_INSTALL_ARGS arch=$ARCH
